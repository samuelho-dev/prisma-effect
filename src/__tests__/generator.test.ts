...
describe('Prisma Effect Schema Generator - E2E', () => {
  ...
  describe('Type Mapping', () => {
    ...
    it('should map optional String to Schema.optional(Schema.String)', () => {
      expect(typesContent).toMatch(
        /optionalString:\s*Schema\.optional\(Schema\.String\)/,
      );
    });
  ...
  describe('UUID Detection', () => {
    ...
    it('should detect optional UUID fields', () => {
      ...
      expect(typesContent).toMatch(
        /Employee[\s\S]*?managerId:\s*Schema\.optional\(Schema\.UUID\)/,
      );
    });
