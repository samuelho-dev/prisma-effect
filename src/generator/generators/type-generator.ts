...
export type ${operationalSchemaName}Update = Schema.Schema.Type<typeof ${operationalSchemaName}.Updateable>;`;
  ...
  private generateModelFields(model: DMMF.Model) {
    return [...model.fields]
      ...
      .sort((a, b) => a.name.localeCompare(b.name)) // Sort alphabetically
      .map((field) => {
        const fieldType = this.typeMapper.getEffectSchemaType(field);

        // If field has @map directive, use propertySignature with fromKey
        if (field.dbName && field.dbName !== field.name) {
          return `  ${field.name}: Schema.propertySignature(${fieldType}).pipe(Schema.fromKey("${field.dbName}"))`;
        }
